distribution_function <- function(distType, outcome) {
  ...
  # Gamma Distribution
    ...
    modelFit <- rgamma(10000,
      shape = fitDistModel$estimate[[1]],
...
create_plot <- function(outcome, dataset, fit, fitDistModel, distType) {
  my_plot <- plot_ly(
    x = ~ unlist(dataset[, outcome]), type = "histogram",
    histnorm = "probability", name = "Data Histogram"
  ) %>%
    add_trace(
      x = ~ fit$x, y = ~ fit$y, type = "scatter",
      mode = "lines", opacity = 0.1, fill = "tozeroy",
      name = modelString(fitDistModel, distType)
    ) %>%
    layout(
      title = "Data Histogram & Probability Distribution Model",
...
modelString <- function(fitDistModel, distType) {
  ...
  if (num == 0) {
    ...
    while (num != 0) {
      result <- paste0(
        result, names(fitDistModel$estimate)[tempnum + 1], "=",
