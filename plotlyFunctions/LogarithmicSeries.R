plotlyLogarithmicSeriesDistribution <- function(plotrange, input, distType, probrange, session) {
    ...
    xseq <- seq(round(max(0, as.numeric(plotrange[1])), 0), round(max(as.numeric(plotrange[2]),
        10), 0), 1)
    f43 <- 0
    graphtype <- ""
    ...
    if (graphtype != "") {
        ...
        fig <- plot_ly(x = xseq, y = f43, name = distType, type = "bar", marker = list(color = colors),
            text = f43, hovertemplate = paste("<br><b>Prob. </b>: %{y}</br>", "<b>X</b>: %{x}",
                "<b>Y</b>: %{y}"), )
        fig <- fig %>%
            plotly::layout(title = paste(distributions[43], " - ", graphtype, sep = ""),
