plotlyVonMisesDistribution <- function(plotrange, input, distType, probrange, session) {
    ...
    if (graphtype != "") {
        fig <- plot_ly(x = xseq, y = f71, name = distType, type = "scatter", mode = "lines",
            hoverinfo = "xy")
        xsize <- length(xseq)
        newy <- f71
        ...
        prob <- circular::pvonmises(as.numeric(probrange[2]), as.numeric(input$vonMisesMu),
            as.numeric(input$vonMisesKappa)) - circular::pvonmises(as.numeric(probrange[1]),
        ...
        fig <- fig %>%
            plotly::layout(title = paste(distributions[15], " - ", graphtype, sep = ""),
                hovermode = "x", hoverlabel = list(namelength = 100), yaxis = list(fixedrange = TRUE,
                  zeroline = TRUE, range = c(min(f71), max(f71))), xaxis = list(showticklabels = TRUE,
