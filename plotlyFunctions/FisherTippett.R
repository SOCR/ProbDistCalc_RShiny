plotlyFisherTippettDistribution <- function(plotrange, input, distType, probrange) {
        ...
        if(input$FunctionType == "PDF/PMF"){
          f26<-dgev(xseq, as.numeric(input$GEVMiu), as.numeric(input$GEVSigma), as.numeric(input$GEVEpsilon) )
          graphtype<-"PMF"
        }
        else if(input$FunctionType == "CDF/CMF"){
          f26<-pgev(xseq, as.numeric(input$GEVMiu), as.numeric(input$GEVSigma), as.numeric(input$GEVEpsilon) )
          graphtype<-"CMF"
        }
        else{
          graphtype<-""
        ...
        if(graphtype != ""){
          ...
          fig<-fig %>% plotly::layout(title = paste(distributions[26],' - ',graphtype,sep = ""),
                              hovermode  = 'x',
                              hoverlabel = list(
                                namelength = 100
                              ),
