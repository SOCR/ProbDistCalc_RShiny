plotlyRiceRicianDistribution <- function(plotrange, input, distType, probrange) {
    ...
    if (graphtype != "") {
        fig <- plot_ly(x = xseq, y = f65, name = distType, type = "scatter", mode = "lines",
            hoverinfo = "xy")
        xsize = length(xseq)
        newy = f65
        ...
        fig <- fig %>%
            plotly::layout(title = paste(distributions[65], " - ", graphtype, sep = ""),
                hovermode = "x", hoverlabel = list(namelength = 100), yaxis = list(fixedrange = TRUE,
                  zeroline = TRUE, range = c(min(f65), max(f65))), xaxis = list(showticklabels = TRUE,
                  zeroline = TRUE, showline = TRUE, showgrid = TRUE, linecolor = "rgb(204, 204, 204)",
                  linewidth = 2, mirror = FALSE, fixedrange = TRUE, range = c(plotrange[1],
