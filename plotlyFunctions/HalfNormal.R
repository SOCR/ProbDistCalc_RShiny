plotlyHalfNormalDistribution <- function(plotrange, input, distType, probrange, session) {
    ...
    if (graphtype != "") {
        ...
        prob = phnorm(as.numeric(probrange[2]), as.numeric(input$HNorm)) - phnorm(as.numeric(probrange[1]),
            as.numeric(input$HNorm))
        fig <- fig %>%
            add_trace(x = xseq, y = newy, name = paste("Probability = ", prob, sep = ""),
                hoverinfo = "name", fill = "tozeroy", fillcolor = "rgba(255, 212, 96, 0.5)")
        fig <- fig %>%
            plotly::layout(title = paste(distributions[34], " - ", graphtype, sep = ""),
                hovermode = "x", hoverlabel = list(namelength = 100), yaxis = list(fixedrange = TRUE,
                  zeroline = TRUE, range = c(min(f34), max(f34))), xaxis = list(showticklabels = TRUE,
