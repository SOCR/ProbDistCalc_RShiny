plotlyHalfNormalDistribution <- function(plotrange, input, distType, probrange, session) {
    ...
    if (graphtype != "") {
        fig <- plot_ly(
            x = xseq, y = f34, name = distType, type = "scatter", mode = "lines",
            hoverinfo = "xy"
        )
        xsize <- length(xseq)
        newy <- f34
        ...
        prob <- phnorm(as.numeric(probrange[2]), as.numeric(input$HNorm)) - phnorm(
        ...
        fig <- fig %>%
            plotly::layout(
                title = paste(distributions[34], " - ", graphtype, sep = ""),
                hovermode = "x", hoverlabel = list(namelength = 100), yaxis = list(
                    fixedrange = TRUE,
                    zeroline = TRUE, range = c(min(f34), max(f34))
                ), xaxis = list(
                    showticklabels = TRUE,
