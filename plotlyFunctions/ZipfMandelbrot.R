plotlyZipfMandelbrotDistribution <- function(plotrange, input, distType, probrange) {
    ...
    if (input$FunctionType == "PDF/PMF") {
        f75 <- dzipfman(xseq, as.numeric(input$Zipf_s), as.numeric(input$Zipf_q),
            as.numeric(input$Zipf_N))
        graphtype <- "PMF"
    } else if (input$FunctionType == "CDF/CMF") {
        f75 <- pzipfman(xseq, as.numeric(input$Zipf_s), as.numeric(input$Zipf_q),
            as.numeric(input$Zipf_N))
        graphtype <- "CMF"
    } else {
        graphtype <- ""
    ...
    if (graphtype != "") {
        ...
        fig <- plot_ly(x = xseq, y = f75, name = distType, type = "bar", marker = list(color = colors),
            text = f75, hovertemplate = paste("<br><b>Prob. </b>: %{y}</br>", "<b>X</b>: %{x}",
                "<b>Y</b>: %{y}"), )
        fig <- fig %>%
            plotly::layout(title = paste(distributions[75], " - ", graphtype, sep = ""),
                hovermode = "x", hoverlabel = list(namelength = 100), yaxis = list(fixedrange = TRUE,
                  zeroline = TRUE, range = c(min(f75), max(f75)), type = "linear"),
