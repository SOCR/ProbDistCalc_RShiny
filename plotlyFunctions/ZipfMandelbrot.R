plotlyZipfMandelbrotDistribution <- function(plotrange, input, distType, probrange) {
    ...
    if (input$FunctionType == "PDF/PMF") {
        f75 <- dzipfman(
            xseq, as.numeric(input$Zipf_s), as.numeric(input$Zipf_q),
    ...
    if (graphtype != "") {
        ...
        fig <- plot_ly(
            x = xseq, y = f75, name = distType, type = "bar", marker = list(color = colors),
            text = f75, hovertemplate = paste(
        ...
        fig <- fig %>%
            plotly::layout(
                title = paste(distributions[75], " - ", graphtype, sep = ""),
