plotlyFisherFDistribution <- function(plotrange, input, distType, probrange, session) {
        ...
        if (plotrange[2] != mean + as.numeric(input$SDNum) * standard_dev && input$numericalValues ==
            ...
            if (input$SDNum > 0) {
                updateSliderInput(session, "plotrange",
                    label = NULL, value = c(
                        0,
                        mean + as.numeric(input$SDNum) * standard_dev
                    ), min = 0, max = mean +
    ...
    if (graphtype != "") {
        fig <- plot_ly(
            x = xseq, y = f25, name = distType, type = "scatter", mode = "lines",
            hoverinfo = "xy"
        )
        xsize <- length(xseq)
        newy <- f25
        ...
        prob <- pf(as.numeric(probrange[2]), as.numeric(input$FdOne), as.numeric(input$FdTwo)) -
            pf(as.numeric(probrange[1]), as.numeric(input$FdOne), as.numeric(input$FdTwo))
        fig <- fig %>%
            add_trace(
        ...
        fig <- fig %>%
            plotly::layout(
                title = paste(distributions[25], " - ", graphtype, sep = ""),
                hovermode = "x", hoverlabel = list(namelength = 100), yaxis = list(
                    fixedrange = TRUE,
                    zeroline = TRUE, range = c(min(f25), max(f25))
                ), xaxis = list(
                    showticklabels = TRUE,
