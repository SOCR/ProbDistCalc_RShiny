plotlyBenfordDistribution <- function(plotrange, input, distType, probrange) {
    ...
    if (graphtype != "") {
        ...
        fig <- plot_ly(x = xseq, y = f3, name = distType, type = "bar", marker = list(color = colors),
            text = f3, hovertemplate = paste("<br><b>Prob. </b>: %{y}</br>", "<b>X</b>: %{x}",
                "<b>Y</b>: %{y}"), )
        fig <- fig %>%
            plotly::layout(title = paste(distributions[3], " - ", graphtype, sep = ""),
                hovermode = "x", hoverlabel = list(namelength = 100), yaxis = list(fixedrange = TRUE,
                  zeroline = TRUE, range = c(min(f3), max(f3)), type = "linear"),
