plotlyNormalTruncatedDistribution <- function(plotrange, input, distType, probrange) {
    ...
    if (input$FunctionType == "PDF/PMF") {
        f58 <- dtruncnorm(xseq, as.numeric(input$TruncNormMin), as.numeric(input$TruncNormMax),
            as.numeric(input$TruncNormMean), as.numeric(input$TruncNormSD))
        graphtype <- "PDF"
    } else if (input$FunctionType == "CDF/CMF") {
        f58 <- ptruncnorm(xseq, as.numeric(input$TruncNormMin), as.numeric(input$TruncNormMax),
            as.numeric(input$TruncNormMean), as.numeric(input$TruncNormSD))
        graphtype <- "CDF"
    } else {
        graphtype <- ""
    ...
    if (graphtype != "") {
        fig <- plot_ly(x = xseq, y = f58, name = distType, type = "scatter", mode = "lines",
            hoverinfo = "xy")
        xsize = length(xseq)
        newy = f58
        ...
        prob = ptruncnorm(as.numeric(probrange[2]), as.numeric(input$TruncNormMin),
            ...
            ptruncnorm(as.numeric(probrange[1]), as.numeric(input$TruncNormMin),
        ...
        fig <- fig %>%
            plotly::layout(title = paste(distributions[58], " - ", graphtype, sep = ""),
                hovermode = "x", hoverlabel = list(namelength = 100), yaxis = list(fixedrange = TRUE,
                  zeroline = TRUE, range = c(min(f58), max(f58))), xaxis = list(showticklabels = TRUE,
