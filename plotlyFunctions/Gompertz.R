plotlyGompertzDistribution <- function(plotrange, input, distType, probrange, session) {
    ...
    if (graphtype != "") {
        fig <- plot_ly(
            x = xseq, y = f32, name = distType, type = "scatter", mode = "lines",
            hoverinfo = "xy"
        )
        xsize <- length(xseq)
        newy <- f32
        ...
        prob <- pgompertz(
            as.numeric(plotrange[2]), as.numeric(input$Gompertz_N),
            as.numeric(input$Gompertz_B)
        ) - pgompertz(
            as.numeric(plotrange[1]), as.numeric(input$Gompertz_N),
            as.numeric(input$Gompertz_B)
        )
        fig <- fig %>%
            add_trace(
        ...
        fig <- fig %>%
            plotly::layout(
                title = paste(distributions[33], " - ", graphtype, sep = ""),
                hovermode = "x", hoverlabel = list(namelength = 100), yaxis = list(
                    fixedrange = TRUE,
                    zeroline = TRUE, range = c(min(f32), max(f32))
                ), xaxis = list(
                    showticklabels = TRUE,
