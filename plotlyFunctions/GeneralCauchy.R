...
plotlyGeneralCauchyDistribution <- function(plotrange, input, distType, probrange) {
    ...
    if (graphtype != "") {
        fig <- plot_ly(x = xseq, y = f28, name = distType, type = "scatter", mode = "lines",
            hoverinfo = "xy")
        xsize = length(xseq)
        newy = f28
        ...
        fig <- fig %>%
            plotly::layout(title = paste(distributions[28], " - ", graphtype, sep = ""),
                hovermode = "x", hoverlabel = list(namelength = 100), yaxis = list(fixedrange = TRUE,
                  zeroline = TRUE, range = c(min(f28), max(f28))), xaxis = list(showticklabels = TRUE,
