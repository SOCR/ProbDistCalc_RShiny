plotlyBetaBinomialDistribution <- function(plotrange, input, distType, probrange) {
    ...
    if (input$FunctionType == "PDF/PMF") {
        f7 <- dbbinom(
            xseq, as.numeric(input$BetaBinomN), as.numeric(input$BetaBinomU),
    ...
    if (graphtype != "") {
        ...
        fig <- plot_ly(
            x = xseq, y = f7, name = distType, type = "bar", marker = list(color = colors),
            text = f7, hovertemplate = paste(
        ...
        fig <- fig %>%
            plotly::layout(
                title = paste(distributions[7], " - ", graphtype, sep = ""),
