plotlyDiscreteUniformDistribution <- function(plotrange, input, distType, probrange) {
    ...
    if (graphtype != "") {
        ...
        fig <- plot_ly(x = xseq, y = f20, name = distType, type = "bar", marker = list(color = colors),
            text = f20, hovertemplate = paste("<br><b>Prob. </b>: %{y}</br>", "<b>X</b>: %{x}",
                "<b>Y</b>: %{y}"), )
        fig <- fig %>%
            plotly::layout(title = paste(distributions[20], " - ", graphtype, sep = ""),
                hovermode = "x", hoverlabel = list(namelength = 100), yaxis = list(fixedrange = TRUE,
                  zeroline = TRUE, range = c(min(f20), max(f20)), type = "linear"),
