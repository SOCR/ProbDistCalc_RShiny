...
plotlyMatchingDistribution  <- function(plotrange, input, distType, probrange) {
    ...
    if (graphtype != "") {
        fig <- plot_ly(x = xseq, y = f48, name = distType, type = "scatter", mode = "lines",
            hoverinfo = "xy")
        xsize = length(xseq)
        newy = f48
        ...
        fig <- fig %>%
            plotly::layout(title = paste(distributions[48], " - ", graphtype, sep = ""),
                hovermode = "x", hoverlabel = list(namelength = 100), yaxis = list(fixedrange = TRUE,
                  zeroline = TRUE, range = c(min(f48), max(f48))), xaxis = list(showticklabels = TRUE,
