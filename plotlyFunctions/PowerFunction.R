plotlyPowerFunctionDistribution <- function(plotrange, input, distType, probrange) {
    ...
    if (graphtype != "") {
        fig <- plot_ly(
            x = xseq, y = f63, name = distType, type = "scatter", mode = "lines",
            hoverinfo = "xy"
        )
        xsize <- length(xseq)
        newy <- f63
        ...
        prob <- ppower(
            round(as.numeric(probrange[2]), 0), as.numeric(input$PowerAlpha),
            as.numeric((input$PowerBeta))
        ) - ppower(round(
            as.numeric(probrange[1]),
            0
        ), as.numeric(input$PowerAlpha), as.numeric((input$PowerBeta)))
        ...
        fig <- fig %>%
            plotly::layout(
                title = paste(distributions[63], " - ", graphtype, sep = ""),
                hovermode = "x", hoverlabel = list(namelength = 100), yaxis = list(
                    fixedrange = TRUE,
                    zeroline = TRUE, range = c(min(f63), max(f63))
                ), xaxis = list(
                    showticklabels = TRUE,
