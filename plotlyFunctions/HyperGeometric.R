plotlyHyperGeometricDistribution <- function(
    ...
    if (graphtype != "") {
        ...
        fig <- plot_ly(
            x = xseq, y = f35, name = distType, type = "bar", marker = list(color = colors),
            text = f35, hovertemplate = paste(
        ...
        fig <- fig %>%
            plotly::layout(
                title = paste(distributions[35], " - ", graphtype, sep = ""),
