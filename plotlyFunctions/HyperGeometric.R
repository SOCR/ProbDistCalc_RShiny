plotlyHyperGeometricDistribution <- function(plotrange, input, distType, probrange, session, old_SD) {
    ...
    if (input$FunctionType == "PDF/PMF") {
        f35 <- dhyper(xseq, as.numeric(input$HyperM), as.numeric(input$HyperN) -
            as.numeric(input$HyperM), as.numeric(input$HyperK))
        graphtype <- "PMF"
    } else if (input$FunctionType == "CDF/CMF") {
        f35 <- phyper(xseq, as.numeric(input$HyperM), as.numeric(input$HyperN) -
            as.numeric(input$HyperM), as.numeric(input$HyperK))
        graphtype <- "CMF"
    } else {
        graphtype <- ""
    ...
    if (graphtype != "") {
        ...
        fig <- plot_ly(x = xseq, y = f35, name = distType, type = "bar", marker = list(color = colors),
            text = f35, hovertemplate = paste("<br><b>Prob. </b>: %{y}</br>", "<b>X</b>: %{x}",
                "<b>Y</b>: %{y}"), )
        fig <- fig %>%
            plotly::layout(title = paste(distributions[35], " - ", graphtype, sep = ""),
                hovermode = "x", hoverlabel = list(namelength = 100), yaxis = list(fixedrange = TRUE,
                  zeroline = TRUE, range = c(min(f35), max(f35)), type = "linear"),
