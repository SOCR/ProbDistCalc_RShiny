plotlyStudentsTDistribution <- function(plotrange, input, distType, probrange) {
    ...
    if (graphtype != "") {
        fig <- plot_ly(
            x = xseq, y = f66, name = distType, type = "scatter", mode = "lines",
            hoverinfo = "xy"
        )
        xsize <- length(xseq)
        newy <- f66
        ...
        prob <- pt(as.numeric(probrange[2]), as.numeric(input$Tdof)) - pt(
        ...
        fig <- fig %>%
            plotly::layout(
                title = paste(distributions[66], " - ", graphtype, sep = ""),
                hovermode = "x", hoverlabel = list(namelength = 100), yaxis = list(
                    fixedrange = TRUE,
                    zeroline = TRUE, range = c(min(f66), max(f66))
                ), xaxis = list(
                    showticklabels = TRUE,
