plotlyInverseGammaDistribution <- function(
    ...
    if (graphtype != "") {
        fig <- plot_ly(
            x = xseq, y = f37, name = distType, type = "scatter", mode = "lines",
            hoverinfo = "xy"
        )
        xsize <- length(xseq)
        newy <- f37
        ...
        fig <- fig %>%
            plotly::layout(
                title = paste(distributions[37], " - ", graphtype, sep = ""),
                hovermode = "x", hoverlabel = list(namelength = 100), yaxis = list(
                    fixedrange = TRUE,
                    zeroline = TRUE, range = c(min(f37), max(f37))
                ), xaxis = list(
                    showticklabels = TRUE,
