plotlyJohnsonSUUnboundedDistribution <- function(plotrange, input, distType, probrange) {
    ...
    if (input$FunctionType == "PDF/PMF") {
        f39 <- dJohnsonSU(
            xseq, as.numeric(input$JohnSUgamma), as.numeric(input$JohnSUdelta),
    ...
    if (graphtype != "") {
        ...
        prob <- pJohnsonSU(
            as.numeric(probrange[2]), as.numeric(input$JohnSUgamma),
            as.numeric(input$JohnSUdelta), as.numeric(input$JohnSUxi), as.numeric(input$JohnSUlambda)
        ) -
            pJohnsonSU(
                as.numeric(probrange[1]), as.numeric(input$JohnSUgamma), as.numeric(input$JohnSUdelta),
                as.numeric(input$JohnSUxi), as.numeric(input$JohnSUlambda)
            )
        fig <- fig %>%
            add_trace(
