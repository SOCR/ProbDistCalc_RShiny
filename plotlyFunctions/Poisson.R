plotlyPoissonDistribution <- function(plotrange, input, distType, probrange, session) {
    ...
    if (graphtype != "") {
        ...
        fig <- plot_ly(x = xseq, y = f61, name = distType, type = "bar", marker = list(color = colors),
            text = f61, hovertemplate = paste("<br><b>Prob. </b>: %{y}</br>", "<b>X</b>: %{x}",
                "<b>Y</b>: %{y}"), )
        fig <- fig %>%
            plotly::layout(title = paste(distributions[61], " - ", graphtype, sep = ""),
