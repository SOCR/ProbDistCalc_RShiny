plotlyInverseGaussianWaldDistribution <- function(plotrange, input, distType, probrange, session) {
    ...
    if (graphtype != "") {
        fig <- plot_ly(x = xseq, y = f38, name = distType, type = "scatter", mode = "lines",
            hoverinfo = "xy")
        xsize = length(xseq)
        newy = f38
        ...
        prob = pinvgauss(as.numeric(probrange[2]), as.numeric(input$InvGausM), as.numeric(input$InvGausL)) -
            pinvgauss(as.numeric(probrange[1]), as.numeric(input$InvGausM), as.numeric(input$InvGausL))
        fig <- fig %>%
            add_trace(x = xseq, y = newy, name = paste("Probability = ", prob, sep = ""),
                hoverinfo = "name", fill = "tozeroy", fillcolor = "rgba(255, 212, 96, 0.5)")
        fig <- fig %>%
            plotly::layout(title = paste(distributions[38], " - ", graphtype, sep = ""),
                hovermode = "x", hoverlabel = list(namelength = 100), yaxis = list(fixedrange = TRUE,
                  zeroline = TRUE, range = c(min(f38), max(f38))), xaxis = list(showticklabels = TRUE,
