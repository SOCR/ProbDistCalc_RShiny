plotlyInverseGaussianWaldDistribution <- function(
    ...
    if (graphtype != "") {
        fig <- plot_ly(
            x = xseq, y = f38, name = distType, type = "scatter", mode = "lines",
            hoverinfo = "xy"
        )
        xsize <- length(xseq)
        newy <- f38
        ...
        fig <- fig %>%
            plotly::layout(
                title = paste(distributions[38], " - ", graphtype, sep = ""),
                hovermode = "x", hoverlabel = list(namelength = 100), yaxis = list(
                    fixedrange = TRUE,
                    zeroline = TRUE, range = c(min(f38), max(f38))
                ), xaxis = list(
                    showticklabels = TRUE,
