plotlyLogisticDistribution <- function(plotrange, input, distType, probrange) {
    ...
    if (graphtype != "") {
        fig <- plot_ly(x = xseq, y = f44, name = distType, type = "scatter", mode = "lines",
            hoverinfo = "xy")
        xsize = length(xseq)
        newy = f44
        ...
        fig <- fig %>%
            plotly::layout(title = paste(distributions[44], " - ", graphtype, sep = ""),
                hovermode = "x", hoverlabel = list(namelength = 100), yaxis = list(fixedrange = TRUE,
                  zeroline = TRUE, range = c(min(f44), max(f44))), xaxis = list(showticklabels = TRUE,
