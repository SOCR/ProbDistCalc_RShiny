plotlyJohnsonSBBoundedDistribution <- function(plotrange, input, distType, probrange) {
    ...
    if (input$FunctionType == "PDF/PMF") {
        f39 <- dJohnsonSB(xseq, as.numeric(input$JohnSBgamma), as.numeric(input$JohnSBdelta),
            as.numeric(input$JohnSBxi), as.numeric(input$JohnSBlambda))
        graphtype <- "PDF"
    } else if (input$FunctionType == "CDF/CMF") {
        f39 <- pJohnsonSB(xseq, as.numeric(input$JohnSBgamma), as.numeric(input$JohnSBdelta),
            as.numeric(input$JohnSBxi), as.numeric(input$JohnSBlambda))
        graphtype <- "CDF"
    } else {
        graphtype <- ""
    ...
    if (graphtype != "") {
        ...
        prob = pJohnsonSB(as.numeric(probrange[2]), as.numeric(input$JohnSBgamma),
            as.numeric(input$JohnSBdelta), as.numeric(input$JohnSBxi), as.numeric(input$JohnSBlambda)) -
            pJohnsonSB(as.numeric(probrange[1]), as.numeric(input$JohnSBgamma), as.numeric(input$JohnSBdelta),
                as.numeric(input$JohnSBxi), as.numeric(input$JohnSBlambda))
        fig <- fig %>%
