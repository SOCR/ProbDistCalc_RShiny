plotlyMaxwellDistribution <- function(plotrange, input, distType, probrange, session) {
    ...
    if (graphtype != "") {
        fig <- plot_ly(
            x = xseq, y = f49, name = distType, type = "scatter", mode = "lines",
            hoverinfo = "xy"
        )
        xsize <- length(xseq)
        newy <- f49
        ...
        prob <- pmaxwell(as.numeric(probrange[2]), as.numeric(input$MaxwellA)) - pmaxwell(
        ...
        fig <- fig %>%
            plotly::layout(
                title = paste(distributions[49], " - ", graphtype, sep = ""),
                hovermode = "x", hoverlabel = list(namelength = 100), yaxis = list(
                    fixedrange = TRUE,
                    zeroline = TRUE, range = c(min(f49), max(f49))
                ), xaxis = list(
                    showticklabels = TRUE,
