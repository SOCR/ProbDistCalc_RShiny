plotlyNegativeHyperGeometricDistribution <- function(plotrange, input, distType,
    probrange) {
    ...
    if (input$FunctionType == "PDF/PMF") {
        f55 <- dnhyper(xseq, as.numeric(input$NegHyperK) - as.numeric(input$NegHyperN),
            as.numeric(input$NegHyperN), as.numeric(input$NegHyperR))
        graphtype <- "PMF"
    } else if (input$FunctionType == "CDF/CMF") {
        f55 <- pnhyper(xseq, as.numeric(input$NegHyperK) - as.numeric(input$NegHyperN),
            as.numeric(input$NegHyperN), as.numeric(input$NegHyperR))
        graphtype <- "CMF"
    } else {
        graphtype <- ""
    ...
    if (graphtype != "") {
        ...
        fig <- plot_ly(x = xseq, y = f55, name = distType, type = "bar", marker = list(color = colors),
            text = f55, hovertemplate = paste("<br><b>Prob. </b>: %{y}</br>", "<b>X</b>: %{x}",
                "<b>Y</b>: %{y}"), )
        fig <- fig %>%
            plotly::layout(title = paste(distributions[55], " - ", graphtype, sep = ""),
