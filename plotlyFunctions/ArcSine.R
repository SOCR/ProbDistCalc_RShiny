plotlyArcSineDistribution <- function(plotrange, input, distType, probrange) {
    ...
    if (graphtype != "") {
        fig <- plot_ly(x = xseq, y = f2, name = distType, type = "scatter", mode = "lines",
            hoverinfo = "xy")
        xsize = length(xseq)
        newy = f2
        ...
        fig <- fig %>%
            plotly::layout(title = paste(distributions[2], " - ", graphtype, sep = ""),
                autosize = TRUE, hovermode = "x", hoverlabel = list(namelength = 100),
                yaxis = list(fixedrange = TRUE, zeroline = TRUE, range = c(min(f2),
                  max(f2))), xaxis = list(showticklabels = TRUE, zeroline = TRUE,
