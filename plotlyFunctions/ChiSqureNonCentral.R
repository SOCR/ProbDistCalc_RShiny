plotlyChiSquareNonCentralDistribution <- function(
    ...
    if (graphtype != "") {
        fig <- plot_ly(
            x = xseq, y = f14, name = distType, type = "scatter", mode = "lines",
            hoverinfo = "xy"
        )
        xsize <- length(xseq)
        newy <- f14
        ...
        fig <- fig %>%
            plotly::layout(
                title = paste(distributions[14], " - ", graphtype, sep = ""),
                hovermode = "x", hoverlabel = list(namelength = 100), yaxis = list(
                    fixedrange = TRUE,
                    zeroline = TRUE, range = c(min(f14), max(f14))
                ), xaxis = list(
                    showticklabels = TRUE,
