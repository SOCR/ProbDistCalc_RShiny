plotlyTriangularDistribution <- function(plotrange, input, distType, probrange) {
    ...
    if (input$FunctionType == "PDF/PMF") {
        f68 <- dtriangular(
            xseq, as.numeric(input$Triangular_A), as.numeric(input$Triangular_B),
    ...
    if (graphtype != "") {
        fig <- plot_ly(
            x = xseq, y = f68, name = distType, type = "scatter", mode = "lines",
            hoverinfo = "xy"
        )
        xsize <- length(xseq)
        newy <- f68
        ...
        prob <- ptriangular(
            as.numeric(probrange[2]), as.numeric(input$Triangular_A),
            as.numeric(input$Triangular_B)
        ) - ptriangular(
        ...
        fig <- fig %>%
            plotly::layout(
                title = paste(distributions[68], " - ", graphtype, sep = ""),
                hovermode = "x", hoverlabel = list(namelength = 100), yaxis = list(
                    fixedrange = TRUE,
                    zeroline = TRUE, range = c(min(f68), max(f68))
                ), xaxis = list(
                    showticklabels = TRUE,
